...
from src.MemeLib import MemeImages
...
class MemeGenerator:
    def __init__(self, meme_image:MemeImage, args):
        ...
        self.image = Image.open("MemePy/ImageLibrary/" + meme_image.image_file_name)
    ...
    @staticmethod
    def generator_from_template(template, texts):
        try:
            for m in list(MemeImages):
                if template.lower() == m.name.lower():
